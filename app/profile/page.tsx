  ...
  const loadUserStats = async () => {
    try {
      ...
      if (profile) {
        ...
        // Load clan information - serialize to plain objects
        ...
        if (clanMember?.clans) {
          // Access first element if clans is an array, otherwise use as object
          const clan = Array.isArray(clanMember.clans) ? clanMember.clans[0] : clanMember.clans;
          
          // Serialize to plain object to avoid Server Component errors
          setClanInfo(JSON.parse(JSON.stringify({
            ...
            clan_total_xp: clan?.total_xp || 0
          })));
        }

        // Load recent habit completions for activity - serialize to plain objects
        ...
        if (recentLogs) {
          ...
          setRecentActivity(JSON.parse(JSON.stringify(recentLogs)));
        }
      }
    } catch (error) {
